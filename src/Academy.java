...
public class Academy extends BuildingNotLevelUp {
    ...
    protected void Do(Wallet wallet) {
        ...
        if (wallet.getMoney()>=cost)
        {
            ...
            switch (otvet){
                case 1:
                {
                    GeneretedUnit newUnit=new GeneretedUnit(name,Integer.toString(indexOfNewUnit),health,attack,rangeAttack,defense,movement,cost,costOfSwamp,costOfHill,costOfTree);
                    System.out.println("Воин "+name+" создан");
                    newUnits.add((Unit)newUnit);
                    isThereNewUnit=true;
